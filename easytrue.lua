...
function Junkie.check_key(key)
    ...
    if not resp then return {valid = false, error = "ERROR"} end
    if resp.StatusCode ~= 200 then return {valid = false, error = "http " .. resp.StatusCode} end
...
function Junkie.load_script()
    ...
    print("https://api.jnkie.com/api/v1/luascripts/public/" .. tostring(Junkie.script_id) .. "/download")
